...
const SkillsSection: React.FC<SkillsSectionProps> = ({ skills }) => {
 return (
  <div className="bg-gradient-to-br from-slate-800/90 to-slate-700/90 backdrop-blur-xl rounded-lg border border-slate-600/30 shadow-xl overflow-hidden">
    <div className="bg-gradient-to-r from-cyan-800/60 to-teal-700/60 p-4 sm:p-6 border-b border-slate-600/30">
      ...
      {skills && skills.length > 0 ? (
        <div className="grid grid-cols-1 gap-3">
          {skills.map((skillItem, skillIndex) => (
            <div
              key={skillIndex}
              className="group p-3 sm:p-4 bg-slate-700/40 hover:bg-slate-600/40 rounded-lg sm:rounded-xl border border-slate-600/30 hover:border-cyan-600/40 transition-all duration-300 cursor-pointer"
            >
