...
const formFields = [
  ...
  { name: 'password', label: 'Password', type: 'password', placeholder: 'Create a secure password' },
  ...
  { name: 'state', label: 'State/Province', type: 'text', placeholder: 'Your state or province' },
  { name: 'city', label: 'City', type: 'text', placeholder: 'Your city' },
  { name: 'phone', label: 'Phone Number', type: 'text', placeholder: '+1 (555) 123-4567' },
  { name: 'about', label: 'Professional Summary', type: 'textarea', placeholder: 'Brief description of your professional background (optional)', rows: 4 },
];
...
const RegisterView: React.FC = () => {
  ...
  return (
<div className="min-h-[120vh] flex items-center justify-center bg-gradient-to-br from-slate-900 via-emerald-900 to-blue-900 px-4 pt-32 pb-20 relative overflow-hidden">      {/* Background decorative elements */}
      ...
      <div className={`w-full max-w-4xl relative z-10 transform transition-all duration-700 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <div className="relative bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-sm rounded-2xl shadow-2xl p-8 border border-emerald-500/20">
          ...
          <div className="relative mb-8 text-center">
            ...
            <p className="text-slate-300 text-sm font-medium">Join thousands of professionals in our platform</p>
          </div>
            ...
            {({ errors, touched, isSubmitting }) => (
              <Form className={`space-y-6 ${styles.formContainer}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {formFields.map(({ name, label, type, placeholder, options, rows }, index) => {
                    ...
                    return (
                      <div 
                        key={name} 
                        className={`group ${isFullWidth ? 'md:col-span-2' : ''} ${styles.fieldContainer}`}
                        style={{ animationDelay: `${index * 50}ms` }}
                      >
                        <label htmlFor={name} className="block text-slate-200 font-semibold mb-2 text-sm group-hover:text-emerald-400 transition-colors duration-200">
                          {label}
                        </label>
                        ...
                        {type === 'select' ? (
                          ...
                          <div className="relative">
                            ...
                            >
                              {((name === 'password' && showPassword) || (name === 'confirmPassword' && showConfirmPassword)) ? "👁️‍🗨️" : "👁️"}
                            </button>
                          </div>
                        ) : (
                          <Field
                        ...
                        {showError && (
                          <div className={`text-red-400 mt-2 text-sm font-medium ${styles.errorMessage}`}>
                            {errors[key]}
                          </div>
                        )}
                      </div>
                ...
                <div className={`flex items-start space-x-3 p-4 bg-slate-800/50 rounded-lg border border-slate-600/50 ${styles.termsContainer}`}>
                  <div className="flex items-center h-5">
                    <Field type="checkbox" name="termsAccepted" className="peer sr-only" />
                    <label htmlFor="termsAccepted" className="relative flex items-center cursor-pointer">
                  ...
                  <div className="text-sm">
                    <label htmlFor="termsAccepted" className="font-medium text-slate-200 cursor-pointer">
                      I agree to the{' '}
                      <button type="button" className="text-emerald-400 hover:text-emerald-300 underline font-semibold">
                        Terms of Service
                      </button>{' '}
                      and{' '}
                      <button type="button" className="text-blue-400 hover:text-blue-300 underline font-semibold">
                        Privacy Policy
                      </button>
                    </label>
                  </div>
                ...
                {errors.termsAccepted && touched.termsAccepted && (
                  <div className={`text-red-400 text-sm font-medium ${styles.errorMessage}`}>
